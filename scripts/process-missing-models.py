...
uploaded_metadata = utils.get_from_data_portal_api(
    "upload-model-metadata", {"status": "uploaded"}
)

valid_metadata = [
    data for data in uploaded_metadata if int(data["size"]) > MINIMUM_SIZE
]
...
if not sites:
    ...
    subprocess.check_call(
        [interpreter, wrapper, interpreter, script, "-s", "all", subcommand_freeze]
    )
